...
class CorrAttention(Module):
    ...
    def forward(self, corr_map, pos_emb):
        # corr_map: B x N x hw  pos_emb: hw x B x C
        hw, batch_size, _ = pos_emb.shape
        # pos_emb = pos_emb.reshape(-1, self.num_heads * batch_size, self.match_dim)
...
class CATModule(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False,
        ...
        self.match_dim = match_dim        
        self.inner_pe = SinePositionalEncoding(self.match_dim // 2, normalize=True)

        self.use_AiA = use_AiA
        if self.use_AiA:
            self.inner_attn = CorrAttention(num_heads, dropout, self.match_dim, feat_size)
    ...
    def aia_scaled_dot_product_attention(
        ...
        B, Nt, E = q.shape
        q = q / math.sqrt(E)
        # q(B, Nt, E) x k(B, E, Ns) -> (B, Nt, Ns)
        attn = torch.bmm(q, k.transpose(-2, -1))
        # attn_mask = torch.zeros_like(k[:,0,:]).to(torch.bool)
        # pos_emb = self.inner_pe(attn_mask).flatten(2).permute(2,0,1)
        if self.use_AiA:
            corr_map = attn
    ...
    def aia_attention_forward(
        ...
        # Calculate attention and out projection
        attn_output, attn_output_weights = self.aia_scaled_dot_product_attention(q, k, v, attn_mask, dropout_p, pos_emb)
