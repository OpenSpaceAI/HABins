...
import matplotlib
from depth.utils import colorize, write_pfm
    ...
    def show_result(self,
        ...
        if format_only:
            if out_file is not None:
                ...
                # for sintel kitti
                # depth[depth>100] = 100
                # depth = depth.squeeze() * 1000
                # mmcv.imwrite(depth.astype('uint16'), out_file)

                # for viper
                # write_pfm(out_file.replace('.jpg','.pfm'), depth.transpose((1,2,0)).astype(np.float32)) 

        else:
            if out_file is not None:
                depth = colorize(depth, vmin=self.decode_head.min_depth, vmax=self.decode_head.max_depth)
                # depth = colorize(depth, vmin=1e-3, vmax=100)
