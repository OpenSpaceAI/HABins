...
from depth.utils.read_pfm import readPFM
...
class SFDataset(Dataset):
    """SceneFlow dataset for depth estimation. An example of file structure
    is as followed.
    .. code-block:: none
        ├── data
        │   ├── SceneFlow
    ...
    def __init__(self,
        ...
        # self.cam_dir = cam_dir # to be deleted
        ...
        if self.data_root is not None:
            ...
            #     self.cam_dir = osp.join(self.data_root, self.cam_dir) # to be deleted

        # load annotations
        # self.img_infos = self.load_annotations(self.img_dir, self.ann_dir, self.split, self.cam_dir) # delete self.cam_dir
        self.img_infos = self.load_annotations(self.img_dir, self.ann_dir, self.split) # delete self.cam_dir
    ...
    def eval_kb_crop(self, depth_gt):
        ...
        top_margin = int(height - 540)
        left_margin = int((width - 960) / 2)
        depth_cropped = depth_gt[top_margin: top_margin + 540, left_margin: left_margin + 960]
    ...
    def pre_eval(self, preds, indices):
        ...
        for i, (pred, index) in enumerate(zip(preds, indices)):
            ...
            disparity, _ = readPFM(depth_map)
            disparity = (np.asarray(disparity, dtype=np.float32) - 1.) / self.depth_scale
            ...
            depth_map_gt = mmcv.imresize(
                depth_map_gt, (960, 540), return_scale=False)
