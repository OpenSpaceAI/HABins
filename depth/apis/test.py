...
def single_gpu_test(model,
    ...
    for batch_indices, data in zip(loader_indices, data_loader):
        ...
        pred_disps_mono.append(1.0 / result_depth[0] * 0.679)
        ...
        for _ in range(batch_size):
            prog_bar.update()
    output_path = osp.join(out_dir, "disp.npy")
    pred_disps_mono = np.concatenate(pred_disps_mono)
    print("out:", pred_disps_mono.shape)
    np.save(output_path, pred_disps_mono)    
    return results
